...
- 后端健康检查：默认 http://127.0.0.1:8000/api/health

端口说明：

- `8501` 是系统主页面，答辩和演示时打开这个地址。
- `8000` 是后端 API 服务，不是主页面。
- 如果 `8000` 被 Windows 异常残留占用，脚本会自动选择 `8001-8010` 中的可用端口，并在控制台显示实际后端地址。
...
2. 重新运行 `run_demo.bat`。
3. 如果 `8000` 显示被不存在的旧 PID 占用，脚本会自动使用 `8001-8010` 中的备用后端端口。
4. 如果仍失败，查看 `artifacts/run_demo.log`。
